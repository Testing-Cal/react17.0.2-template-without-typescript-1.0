...
pipeline {
  ...
  environment {
    DEFAULT_STAGE_SEQ = "'Initialization','Build','UnitTests','SonarQubeScan','BuildContainerImage','QualysScan','Rapid7Scan','SysdigScan','PublishContainerImage','Deploy','FunctionalTests','Destroy'"
    ...
    RELEASE_NAME = "react17withouttypscript"
           ...
           if(kubeVars['vault']){
             ...
             for (int i = 0; i < list.size(); i++) {
               ...
               if ("${list[i]}" == "'UnitTests'" && env.ACTION == 'DEPLOY') {
                 stage('Unit Tests') {
                    sh """
                    docker run --rm --user root -v "$WORKSPACE":/home/circleci/app $NODE_IMAGE /bin/bash -c "cd /home/circleci/app &&  npm install && npm test -- --watchAll=false && npm run test:coverage"
